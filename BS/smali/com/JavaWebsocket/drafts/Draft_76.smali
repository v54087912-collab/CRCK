# classes10.dex

.class public Lcom/JavaWebsocket/drafts/Draft_76;
.super Lcom/JavaWebsocket/drafts/Draft_75;
.source "Draft_76.java"


# static fields
.field private static final closehandshake:[B


# instance fields
.field private failed:Z

.field private final reuseableRandom:Ljava/util/Random;


# direct methods
.method static constructor <clinit>()V
    .registers 1

    const/4 v0, 0x2

    .line 30
    new-array v0, v0, [B

    fill-array-data v0, :array_a

    sput-object v0, Lcom/JavaWebsocket/drafts/Draft_76;->closehandshake:[B

    return-void

    nop

    :array_a
    .array-data 1
        -0x1t
        0x0t
    .end array-data
.end method

.method public constructor <init>()V
    .registers 2

    .line 28
    invoke-direct {p0}, Lcom/JavaWebsocket/drafts/Draft_75;-><init>()V

    const/4 v0, 0x0

    .line 29
    iput-boolean v0, p0, Lcom/JavaWebsocket/drafts/Draft_76;->failed:Z

    .line 32
    new-instance v0, Ljava/util/Random;

    invoke-direct {v0}, Ljava/util/Random;-><init>()V

    iput-object v0, p0, Lcom/JavaWebsocket/drafts/Draft_76;->reuseableRandom:Ljava/util/Random;

    return-void
.end method

.method public static createChallenge(Ljava/lang/String;Ljava/lang/String;[B)[B
    .registers 27
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;
        }
    .end annotation

    .line 36
    invoke-static/range {p0 .. p0}, Lcom/JavaWebsocket/drafts/Draft_76;->getPart(Ljava/lang/String;)[B

    move-result-object v0

    .line 37
    invoke-static/range {p1 .. p1}, Lcom/JavaWebsocket/drafts/Draft_76;->getPart(Ljava/lang/String;)[B

    move-result-object v1

    const/4 v2, 0x0

    .line 39
    aget-byte v3, v0, v2

    const/4 v4, 0x1

    .line 40
    aget-byte v5, v0, v4

    const/4 v6, 0x2

    .line 41
    aget-byte v7, v0, v6

    const/4 v8, 0x3

    .line 42
    aget-byte v0, v0, v8

    .line 43
    aget-byte v9, v1, v2

    .line 44
    aget-byte v10, v1, v4

    .line 45
    aget-byte v11, v1, v6

    .line 46
    aget-byte v1, v1, v8

    .line 47
    aget-byte v12, p2, v2

    .line 48
    aget-byte v13, p2, v4

    .line 49
    aget-byte v14, p2, v6

    .line 50
    aget-byte v15, p2, v8

    const/16 v16, 0x4

    .line 51
    aget-byte v17, p2, v16

    const/16 v18, 0x5

    .line 52
    aget-byte v19, p2, v18

    const/16 v20, 0x6

    .line 53
    aget-byte v21, p2, v20

    const/16 v22, 0x7

    .line 54
    aget-byte v23, p2, v22

    const/16 v8, 0x10

    new-array v8, v8, [B

    aput-byte v3, v8, v2

    aput-byte v5, v8, v4

    aput-byte v7, v8, v6

    const/4 v2, 0x3

    aput-byte v0, v8, v2

    aput-byte v9, v8, v16

    aput-byte v10, v8, v18

    aput-byte v11, v8, v20

    aput-byte v1, v8, v22

    const/16 v0, 0x8

    aput-byte v12, v8, v0

    const/16 v0, 0x9

    aput-byte v13, v8, v0

    const/16 v0, 0xa

    aput-byte v14, v8, v0

    const/16 v0, 0xb

    aput-byte v15, v8, v0

    const/16 v0, 0xc

    aput-byte v17, v8, v0

    const/16 v0, 0xd

    aput-byte v19, v8, v0

    const/16 v0, 0xe

    aput-byte v21, v8, v0

    const/16 v0, 0xf

    aput-byte v23, v8, v0

    .line 57
    :try_start_69
    const-string v0, "MD5"

    invoke-static {v0}, Ljava/security/MessageDigest;->getInstance(Ljava/lang/String;)Ljava/security/MessageDigest;

    move-result-object v0
    :try_end_6f
    .catch Ljava/security/NoSuchAlgorithmException; {:try_start_69 .. :try_end_6f} :catch_74

    .line 61
    invoke-virtual {v0, v8}, Ljava/security/MessageDigest;->digest([B)[B

    move-result-object v0

    return-object v0

    :catch_74
    move-exception v0

    .line 59
    new-instance v1, Ljava/lang/RuntimeException;

    invoke-direct {v1, v0}, Ljava/lang/RuntimeException;-><init>(Ljava/lang/Throwable;)V

    throw v1
.end method

.method private static generateKey()Ljava/lang/String;
    .registers 10

    .line 65
    new-instance v0, Ljava/util/Random;

    invoke-direct {v0}, Ljava/util/Random;-><init>()V

    const/16 v1, 0xc

    .line 67
    invoke-virtual {v0, v1}, Ljava/util/Random;->nextInt(I)I

    move-result v2

    add-int/lit8 v2, v2, 0x1

    int-to-long v2, v2

    .line 68
    new-instance v4, Ljava/lang/Long;

    const-wide v5, 0xffffffffL

    div-long/2addr v5, v2

    invoke-direct {v4, v5, v6}, Ljava/lang/Long;-><init>(J)V

    invoke-virtual {v4}, Ljava/lang/Long;->intValue()I

    move-result v4

    .line 69
    invoke-static {v4}, Ljava/lang/Math;->abs(I)I

    move-result v4

    .line 70
    invoke-virtual {v0, v4}, Ljava/util/Random;->nextInt(I)I

    move-result v4

    add-int/lit8 v4, v4, 0x1

    int-to-long v4, v4

    mul-long v4, v4, v2

    .line 72
    invoke-static {v4, v5}, Ljava/lang/Long;->toString(J)Ljava/lang/String;

    move-result-object v4

    .line 74
    invoke-virtual {v0, v1}, Ljava/util/Random;->nextInt(I)I

    move-result v1

    add-int/lit8 v1, v1, 0x1

    const/4 v5, 0x0

    const/4 v6, 0x0

    :goto_36
    if-ge v6, v1, :cond_68

    .line 76
    invoke-virtual {v4}, Ljava/lang/String;->length()I

    move-result v7

    invoke-virtual {v0, v7}, Ljava/util/Random;->nextInt(I)I

    move-result v7

    .line 77
    invoke-static {v7}, Ljava/lang/Math;->abs(I)I

    move-result v7

    const/16 v8, 0x5f

    .line 78
    invoke-virtual {v0, v8}, Ljava/util/Random;->nextInt(I)I

    move-result v8

    add-int/lit8 v8, v8, 0x21

    int-to-char v8, v8

    const/16 v9, 0x30

    if-lt v8, v9, :cond_58

    const/16 v9, 0x39

    if-gt v8, v9, :cond_58

    add-int/lit8 v8, v8, -0xf

    int-to-char v8, v8

    .line 83
    :cond_58
    new-instance v9, Ljava/lang/StringBuilder;

    invoke-direct {v9, v4}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v9, v7, v8}, Ljava/lang/StringBuilder;->insert(IC)Ljava/lang/StringBuilder;

    move-result-object v4

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    add-int/lit8 v6, v6, 0x1

    goto :goto_36

    :cond_68
    :goto_68
    int-to-long v6, v5

    cmp-long v1, v6, v2

    if-gez v1, :cond_8f

    .line 86
    invoke-virtual {v4}, Ljava/lang/String;->length()I

    move-result v1

    add-int/lit8 v1, v1, -0x1

    invoke-virtual {v0, v1}, Ljava/util/Random;->nextInt(I)I

    move-result v1

    add-int/lit8 v1, v1, 0x1

    .line 87
    invoke-static {v1}, Ljava/lang/Math;->abs(I)I

    move-result v1

    .line 88
    new-instance v6, Ljava/lang/StringBuilder;

    invoke-direct {v6, v4}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    const-string v4, " "

    invoke-virtual {v6, v1, v4}, Ljava/lang/StringBuilder;->insert(ILjava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v1

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    add-int/lit8 v5, v5, 0x1

    goto :goto_68

    :cond_8f
    return-object v4
.end method

.method private static getPart(Ljava/lang/String;)[B
    .registers 8
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;
        }
    .end annotation

    .line 95
    :try_start_0
    const-string v0, "[^0-9]"

    const-string v1, ""

    invoke-virtual {p0, v0, v1}, Ljava/lang/String;->replaceAll(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Ljava/lang/Long;->parseLong(Ljava/lang/String;)J

    move-result-wide v0

    .line 96
    const-string v2, " "

    invoke-virtual {p0, v2}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object p0

    array-length p0, p0

    const/4 v2, 0x1

    sub-int/2addr p0, v2

    int-to-long v3, p0

    const-wide/16 v5, 0x0

    cmp-long p0, v3, v5

    if-eqz p0, :cond_44

    .line 100
    div-long/2addr v0, v3

    const/16 p0, 0x18

    shr-long v3, v0, p0

    long-to-int v4, v3

    int-to-byte v3, v4

    const/16 v4, 0x8

    shl-long v4, v0, v4

    shr-long/2addr v4, p0

    long-to-int v5, v4

    int-to-byte v4, v5

    const/16 v5, 0x10

    shl-long v5, v0, v5

    shr-long/2addr v5, p0

    long-to-int v6, v5

    int-to-byte v5, v6

    shl-long/2addr v0, p0

    shr-long/2addr v0, p0

    long-to-int p0, v0

    int-to-byte p0, p0

    const/4 v0, 0x4

    .line 101
    new-array v0, v0, [B

    const/4 v1, 0x0

    aput-byte v3, v0, v1

    aput-byte v4, v0, v2

    const/4 v1, 0x2

    aput-byte v5, v0, v1

    const/4 v1, 0x3

    aput-byte p0, v0, v1

    return-object v0

    .line 98
    :cond_44
    new-instance p0, Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;

    const-string v0, "invalid Sec-WebSocket-Key (/key2/)"

    invoke-direct {p0, v0}, Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;-><init>(Ljava/lang/String;)V

    throw p0
    :try_end_4c
    .catch Ljava/lang/NumberFormatException; {:try_start_0 .. :try_end_4c} :catch_4c

    .line 103
    :catch_4c
    new-instance p0, Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;

    const-string v0, "invalid Sec-WebSocket-Key (/key1/ or /key2/)"

    invoke-direct {p0, v0}, Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;-><init>(Ljava/lang/String;)V

    throw p0
.end method


# virtual methods
.method public acceptHandshakeAsClient(Lcom/JavaWebsocket/handshake/ClientHandshake;Lcom/JavaWebsocket/handshake/ServerHandshake;)Lcom/JavaWebsocket/drafts/Draft$HandshakeState;
    .registers 5

    .line 109
    iget-boolean v0, p0, Lcom/JavaWebsocket/drafts/Draft_76;->failed:Z

    if-eqz v0, :cond_7

    .line 110
    sget-object p1, Lcom/JavaWebsocket/drafts/Draft$HandshakeState;->NOT_MATCHED:Lcom/JavaWebsocket/drafts/Draft$HandshakeState;

    return-object p1

    .line 114
    :cond_7
    :try_start_7
    const-string v0, "Sec-WebSocket-Origin"

    invoke-interface {p2, v0}, Lcom/JavaWebsocket/handshake/ServerHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "Origin"

    invoke-interface {p1, v1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_4f

    invoke-virtual {p0, p2}, Lcom/JavaWebsocket/drafts/Draft_76;->basicAccept(Lcom/JavaWebsocket/handshake/Handshakedata;)Z

    move-result v0

    if-nez v0, :cond_20

    goto :goto_4f

    .line 117
    :cond_20
    invoke-interface {p2}, Lcom/JavaWebsocket/handshake/ServerHandshake;->getContent()[B

    move-result-object p2

    if-eqz p2, :cond_49

    .line 118
    array-length v0, p2

    if-eqz v0, :cond_49

    .line 121
    const-string v0, "Sec-WebSocket-Key1"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "Sec-WebSocket-Key2"

    invoke-interface {p1, v1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-interface {p1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getContent()[B

    move-result-object p1

    invoke-static {v0, v1, p1}, Lcom/JavaWebsocket/drafts/Draft_76;->createChallenge(Ljava/lang/String;Ljava/lang/String;[B)[B

    move-result-object p1

    invoke-static {p2, p1}, Ljava/util/Arrays;->equals([B[B)Z

    move-result p1

    if-eqz p1, :cond_46

    .line 122
    sget-object p1, Lcom/JavaWebsocket/drafts/Draft$HandshakeState;->MATCHED:Lcom/JavaWebsocket/drafts/Draft$HandshakeState;

    return-object p1

    .line 124
    :cond_46
    sget-object p1, Lcom/JavaWebsocket/drafts/Draft$HandshakeState;->NOT_MATCHED:Lcom/JavaWebsocket/drafts/Draft$HandshakeState;

    return-object p1

    .line 119
    :cond_49
    new-instance p1, Lcom/JavaWebsocket/exceptions/IncompleteHandshakeException;

    invoke-direct {p1}, Lcom/JavaWebsocket/exceptions/IncompleteHandshakeException;-><init>()V

    throw p1

    .line 115
    :cond_4f
    :goto_4f
    sget-object p1, Lcom/JavaWebsocket/drafts/Draft$HandshakeState;->NOT_MATCHED:Lcom/JavaWebsocket/drafts/Draft$HandshakeState;
    :try_end_51
    .catch Lcom/JavaWebsocket/exceptions/InvalidHandshakeException; {:try_start_7 .. :try_end_51} :catch_52

    return-object p1

    :catch_52
    move-exception p1

    .line 127
    new-instance p2, Ljava/lang/RuntimeException;

    const-string v0, "bad handshakerequest"

    invoke-direct {p2, v0, p1}, Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;Ljava/lang/Throwable;)V

    throw p2
.end method

.method public acceptHandshakeAsServer(Lcom/JavaWebsocket/handshake/ClientHandshake;)Lcom/JavaWebsocket/drafts/Draft$HandshakeState;
    .registers 5

    .line 133
    const-string v0, "Upgrade"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    const-string v2, "WebSocket"

    invoke-virtual {v1, v2}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v1

    if-eqz v1, :cond_3d

    const-string v1, "Connection"

    invoke-interface {p1, v1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v1, v0}, Ljava/lang/String;->contains(Ljava/lang/CharSequence;)Z

    move-result v0

    if-eqz v0, :cond_3d

    const-string v0, "Sec-WebSocket-Key1"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/String;->length()I

    move-result v0

    if-lez v0, :cond_3d

    const-string v0, "Sec-WebSocket-Key2"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/String;->isEmpty()Z

    move-result v0

    if-nez v0, :cond_3d

    const-string v0, "Origin"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->hasFieldValue(Ljava/lang/String;)Z

    move-result p1

    if-eqz p1, :cond_3d

    .line 134
    sget-object p1, Lcom/JavaWebsocket/drafts/Draft$HandshakeState;->MATCHED:Lcom/JavaWebsocket/drafts/Draft$HandshakeState;

    return-object p1

    .line 135
    :cond_3d
    sget-object p1, Lcom/JavaWebsocket/drafts/Draft$HandshakeState;->NOT_MATCHED:Lcom/JavaWebsocket/drafts/Draft$HandshakeState;

    return-object p1
.end method

.method public copyInstance()Lcom/JavaWebsocket/drafts/Draft;
    .registers 2

    .line 240
    new-instance v0, Lcom/JavaWebsocket/drafts/Draft_76;

    invoke-direct {v0}, Lcom/JavaWebsocket/drafts/Draft_76;-><init>()V

    return-object v0
.end method

.method public createBinaryFrame(Lcom/JavaWebsocket/framing/Framedata;)Ljava/nio/ByteBuffer;
    .registers 4

    .line 228
    invoke-interface {p1}, Lcom/JavaWebsocket/framing/Framedata;->getOpcode()Lcom/JavaWebsocket/framing/Framedata$Opcode;

    move-result-object v0

    sget-object v1, Lcom/JavaWebsocket/framing/Framedata$Opcode;->CLOSING:Lcom/JavaWebsocket/framing/Framedata$Opcode;

    if-ne v0, v1, :cond_f

    .line 229
    sget-object p1, Lcom/JavaWebsocket/drafts/Draft_76;->closehandshake:[B

    invoke-static {p1}, Ljava/nio/ByteBuffer;->wrap([B)Ljava/nio/ByteBuffer;

    move-result-object p1

    return-object p1

    .line 230
    :cond_f
    invoke-super {p0, p1}, Lcom/JavaWebsocket/drafts/Draft_75;->createBinaryFrame(Lcom/JavaWebsocket/framing/Framedata;)Ljava/nio/ByteBuffer;

    move-result-object p1

    return-object p1
.end method

.method public getCloseHandshakeType()Lcom/JavaWebsocket/drafts/Draft$CloseHandshakeType;
    .registers 2

    .line 235
    sget-object v0, Lcom/JavaWebsocket/drafts/Draft$CloseHandshakeType;->ONEWAY:Lcom/JavaWebsocket/drafts/Draft$CloseHandshakeType;

    return-object v0
.end method

.method public postProcessHandshakeRequestAsClient(Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;)Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;
    .registers 5

    .line 140
    const-string v0, "WebSocket"

    const-string v1, "Upgrade"

    invoke-interface {p1, v1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 141
    const-string v0, "Connection"

    invoke-interface {p1, v0, v1}, Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 142
    const-string v0, "Sec-WebSocket-Key1"

    invoke-static {}, Lcom/JavaWebsocket/drafts/Draft_76;->generateKey()Ljava/lang/String;

    move-result-object v1

    invoke-interface {p1, v0, v1}, Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 143
    const-string v0, "Sec-WebSocket-Key2"

    invoke-static {}, Lcom/JavaWebsocket/drafts/Draft_76;->generateKey()Ljava/lang/String;

    move-result-object v1

    invoke-interface {p1, v0, v1}, Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 145
    const-string v0, "Origin"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;->hasFieldValue(Ljava/lang/String;)Z

    move-result v1

    if-nez v1, :cond_3d

    .line 146
    new-instance v1, Ljava/lang/StringBuilder;

    const-string v2, "random"

    invoke-direct {v1, v2}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    iget-object v2, p0, Lcom/JavaWebsocket/drafts/Draft_76;->reuseableRandom:Ljava/util/Random;

    invoke-virtual {v2}, Ljava/util/Random;->nextInt()I

    move-result v2

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-interface {p1, v0, v1}, Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    :cond_3d
    const/16 v0, 0x8

    .line 149
    new-array v0, v0, [B

    .line 150
    iget-object v1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->reuseableRandom:Ljava/util/Random;

    invoke-virtual {v1, v0}, Ljava/util/Random;->nextBytes([B)V

    .line 151
    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshakeBuilder;->setContent([B)V

    return-object p1
.end method

.method public postProcessHandshakeResponseAsServer(Lcom/JavaWebsocket/handshake/ClientHandshake;Lcom/JavaWebsocket/handshake/ServerHandshakeBuilder;)Lcom/JavaWebsocket/handshake/HandshakeBuilder;
    .registers 7
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;
        }
    .end annotation

    .line 158
    const-string v0, "WebSocket Protocol Handshake"

    invoke-interface {p2, v0}, Lcom/JavaWebsocket/handshake/ServerHandshakeBuilder;->setHttpStatusMessage(Ljava/lang/String;)V

    .line 159
    const-string v0, "Upgrade"

    const-string v1, "WebSocket"

    invoke-interface {p2, v0, v1}, Lcom/JavaWebsocket/handshake/ServerHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 160
    const-string v0, "Connection"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-interface {p2, v0, v1}, Lcom/JavaWebsocket/handshake/ServerHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 161
    const-string v0, "Origin"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "Sec-WebSocket-Origin"

    invoke-interface {p2, v1, v0}, Lcom/JavaWebsocket/handshake/ServerHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 162
    new-instance v0, Ljava/lang/StringBuilder;

    const-string/jumbo v1, "ws://"

    invoke-direct {v0, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    const-string v1, "Host"

    invoke-interface {p1, v1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-interface {p1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getResourceDescriptor()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    .line 163
    const-string v1, "Sec-WebSocket-Location"

    invoke-interface {p2, v1, v0}, Lcom/JavaWebsocket/handshake/ServerHandshakeBuilder;->put(Ljava/lang/String;Ljava/lang/String;)V

    .line 164
    const-string v0, "Sec-WebSocket-Key1"

    invoke-interface {p1, v0}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    .line 165
    const-string v1, "Sec-WebSocket-Key2"

    invoke-interface {p1, v1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getFieldValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    .line 166
    invoke-interface {p1}, Lcom/JavaWebsocket/handshake/ClientHandshake;->getContent()[B

    move-result-object p1

    if-eqz v0, :cond_64

    if-eqz v1, :cond_64

    if-eqz p1, :cond_64

    .line 167
    array-length v2, p1

    const/16 v3, 0x8

    if-ne v2, v3, :cond_64

    .line 170
    invoke-static {v0, v1, p1}, Lcom/JavaWebsocket/drafts/Draft_76;->createChallenge(Ljava/lang/String;Ljava/lang/String;[B)[B

    move-result-object p1

    invoke-interface {p2, p1}, Lcom/JavaWebsocket/handshake/ServerHandshakeBuilder;->setContent([B)V

    return-object p2

    .line 168
    :cond_64
    new-instance p1, Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;

    const-string p2, "Bad keys"

    invoke-direct {p1, p2}, Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;-><init>(Ljava/lang/String;)V

    throw p1
.end method

.method public translateFrame(Ljava/nio/ByteBuffer;)Ljava/util/List;
    .registers 5
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/nio/ByteBuffer;",
            ")",
            "Ljava/util/List<",
            "Lcom/JavaWebsocket/framing/Framedata;",
            ">;"
        }
    .end annotation

    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/JavaWebsocket/exceptions/InvalidDataException;
        }
    .end annotation

    .line 194
    invoke-virtual {p1}, Ljava/nio/ByteBuffer;->mark()Ljava/nio/Buffer;

    .line 195
    invoke-super {p0, p1}, Lcom/JavaWebsocket/drafts/Draft_75;->translateRegularFrame(Ljava/nio/ByteBuffer;)Ljava/util/List;

    move-result-object v0

    if-nez v0, :cond_68

    .line 197
    invoke-virtual {p1}, Ljava/nio/ByteBuffer;->reset()Ljava/nio/Buffer;

    .line 198
    iget-object v0, p0, Lcom/JavaWebsocket/drafts/Draft_76;->readyframes:Ljava/util/List;

    const/4 v1, 0x1

    .line 199
    iput-boolean v1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->readingState:Z

    .line 200
    iget-object v1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->currentFrame:Ljava/nio/ByteBuffer;

    if-nez v1, :cond_62

    const/4 v1, 0x2

    .line 201
    invoke-static {v1}, Ljava/nio/ByteBuffer;->allocate(I)Ljava/nio/ByteBuffer;

    move-result-object v1

    iput-object v1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->currentFrame:Ljava/nio/ByteBuffer;

    .line 205
    invoke-virtual {p1}, Ljava/nio/ByteBuffer;->remaining()I

    move-result v1

    iget-object v2, p0, Lcom/JavaWebsocket/drafts/Draft_76;->currentFrame:Ljava/nio/ByteBuffer;

    invoke-virtual {v2}, Ljava/nio/ByteBuffer;->remaining()I

    move-result v2

    if-gt v1, v2, :cond_5c

    .line 208
    iget-object v1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->currentFrame:Ljava/nio/ByteBuffer;

    invoke-virtual {v1, p1}, Ljava/nio/ByteBuffer;->put(Ljava/nio/ByteBuffer;)Ljava/nio/ByteBuffer;

    .line 210
    iget-object p1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->currentFrame:Ljava/nio/ByteBuffer;

    invoke-virtual {p1}, Ljava/nio/ByteBuffer;->hasRemaining()Z

    move-result p1

    if-nez p1, :cond_54

    .line 211
    iget-object p1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->currentFrame:Ljava/nio/ByteBuffer;

    invoke-virtual {p1}, Ljava/nio/ByteBuffer;->array()[B

    move-result-object p1

    sget-object v1, Lcom/JavaWebsocket/drafts/Draft_76;->closehandshake:[B

    invoke-static {p1, v1}, Ljava/util/Arrays;->equals([B[B)Z

    move-result p1

    if-eqz p1, :cond_4e

    .line 212
    new-instance p1, Lcom/JavaWebsocket/framing/CloseFrameBuilder;

    const/16 v1, 0x3e8

    invoke-direct {p1, v1}, Lcom/JavaWebsocket/framing/CloseFrameBuilder;-><init>(I)V

    invoke-interface {v0, p1}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    return-object v0

    .line 216
    :cond_4e
    new-instance p1, Lcom/JavaWebsocket/exceptions/InvalidFrameException;

    invoke-direct {p1}, Lcom/JavaWebsocket/exceptions/InvalidFrameException;-><init>()V

    throw p1

    .line 219
    :cond_54
    new-instance p1, Ljava/util/LinkedList;

    invoke-direct {p1}, Ljava/util/LinkedList;-><init>()V

    iput-object p1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->readyframes:Ljava/util/List;

    return-object v0

    .line 206
    :cond_5c
    new-instance p1, Lcom/JavaWebsocket/exceptions/InvalidFrameException;

    invoke-direct {p1}, Lcom/JavaWebsocket/exceptions/InvalidFrameException;-><init>()V

    throw p1

    .line 203
    :cond_62
    new-instance p1, Lcom/JavaWebsocket/exceptions/InvalidFrameException;

    invoke-direct {p1}, Lcom/JavaWebsocket/exceptions/InvalidFrameException;-><init>()V

    throw p1

    :cond_68
    return-object v0
.end method

.method public translateHandshake(Ljava/nio/ByteBuffer;)Lcom/JavaWebsocket/handshake/Handshakedata;
    .registers 6
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/JavaWebsocket/exceptions/InvalidHandshakeException;
        }
    .end annotation

    .line 177
    iget-object v0, p0, Lcom/JavaWebsocket/drafts/Draft_76;->role:Lcom/JavaWebsocket/WebSocket$Role;

    invoke-static {p1, v0}, Lcom/JavaWebsocket/drafts/Draft_76;->translateHandshakeHttp(Ljava/nio/ByteBuffer;Lcom/JavaWebsocket/WebSocket$Role;)Lcom/JavaWebsocket/handshake/HandshakeBuilder;

    move-result-object v0

    .line 179
    const-string v1, "Sec-WebSocket-Key1"

    invoke-interface {v0, v1}, Lcom/JavaWebsocket/handshake/HandshakeBuilder;->hasFieldValue(Ljava/lang/String;)Z

    move-result v1

    if-nez v1, :cond_14

    iget-object v1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->role:Lcom/JavaWebsocket/WebSocket$Role;

    sget-object v2, Lcom/JavaWebsocket/WebSocket$Role;->CLIENT:Lcom/JavaWebsocket/WebSocket$Role;

    if-ne v1, v2, :cond_3d

    :cond_14
    const-string v1, "Sec-WebSocket-Version"

    invoke-interface {v0, v1}, Lcom/JavaWebsocket/handshake/HandshakeBuilder;->hasFieldValue(Ljava/lang/String;)Z

    move-result v1

    if-nez v1, :cond_3d

    .line 180
    iget-object v1, p0, Lcom/JavaWebsocket/drafts/Draft_76;->role:Lcom/JavaWebsocket/WebSocket$Role;

    sget-object v2, Lcom/JavaWebsocket/WebSocket$Role;->SERVER:Lcom/JavaWebsocket/WebSocket$Role;

    const/16 v3, 0x10

    if-ne v1, v2, :cond_27

    const/16 v1, 0x8

    goto :goto_29

    :cond_27
    const/16 v1, 0x10

    :goto_29
    new-array v1, v1, [B

    .line 182
    :try_start_2b
    invoke-virtual {p1, v1}, Ljava/nio/ByteBuffer;->get([B)Ljava/nio/ByteBuffer;
    :try_end_2e
    .catch Ljava/nio/BufferUnderflowException; {:try_start_2b .. :try_end_2e} :catch_32

    .line 186
    invoke-interface {v0, v1}, Lcom/JavaWebsocket/handshake/HandshakeBuilder;->setContent([B)V

    goto :goto_3d

    .line 184
    :catch_32
    new-instance v0, Lcom/JavaWebsocket/exceptions/IncompleteHandshakeException;

    invoke-virtual {p1}, Ljava/nio/ByteBuffer;->capacity()I

    move-result p1

    add-int/2addr p1, v3

    invoke-direct {v0, p1}, Lcom/JavaWebsocket/exceptions/IncompleteHandshakeException;-><init>(I)V

    throw v0

    :cond_3d
    :goto_3d
    return-object v0
.end method
